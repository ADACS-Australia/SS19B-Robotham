...
for (i in 1:n) {
  ...
  selectOLD=magcutout(image_big, loc=loc, box=box)$image
  selectNEW=magcutoutADACSInPlace(image_big, loc=loc, box=box, ooselect)$image
  if (!all(compareNA(selectOLD,selectNEW)))
  {
    print(paste("Problem at ", loc))
  }
}

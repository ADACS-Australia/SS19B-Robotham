...
profoundImBlur=function(image=NULL, sigma=1, plot=FALSE, ...){
  if(requireNamespace("imager", quietly = TRUE)){
    output=as.matrix(imager::isoblur(imager::as.cimg(image),sigma,na.rm=doRMNA))
  }else{
...
profoundImGrad=function(image=NULL, sigma=1, plot=FALSE, ...){
  ...
  output=as.matrix(imager::enorm(imager::imgradient(imager::isoblur(imager::as.cimg(image),sigma,na.rm=doRMNA), "xy")))
  if(plot){
    magimage(output, ...)
  }
  invisible(output)
...
profoundImDiff=function(image=NULL,sigma=1, plot=FALSE, ...){
  ...
  blur=as.matrix(imager::isoblur(imager::as.cimg(image),sigma,na.rm=doRMNA))
  output=image-blur
  if(plot){
    magimage(output, ...)
  }
